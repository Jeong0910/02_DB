...
-- 제공되는 값외에는 NULL을 사용할 수도 있다

-- 컬럼레벨일 경우
-- 컬럼명 자료형(크기) [CONSTRAINT 이름] REFERENCES 참조할테이블명 [(참조할럴럼명)] [삭제룰]

-- 테이블레벨일 경우
-- [CONSTRINT 이름] FORIGN KEY (적용할컬럼명)
-- REFERENCES 참조할테이블명 [(참조할럴럼명)] [삭제룰]

-- * 참조될 수 있는 컬럼은 PRIMARY KEY컬럼과, UNIQUE 지정된 컬럼만 외래키로 사용 가능.
...
-- USER_GRADE(부모테이블/참조테이블)에 NULL이라는 GRADE_CODE가 존재하므로
-- 자식테이블이 NULL이라는 값 사용 가능.

INSERT INTO USER_USED_FK VALUES
(5, 'USER05', 'PASS05', '윤봉길', 
'남자', '010-6666-7777', 'yoon@kr.or.kr',50);
-- 제약조건(KH_JSW.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
...
-- 제약조건(KH_JSW.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다

DELETE FROM USER_GRADE WHERE GRADE_CODE = 20;
-- GRADE_CODE 중 20을 외래키로 참조하고 있는 곳이 없어서 삭제가능
...
-- 부모키 삭제시 값을 사용하고 있던 자식 테이블의 행이 삭제됨
