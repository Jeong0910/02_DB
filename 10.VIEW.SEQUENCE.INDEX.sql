...
-- VIEW에 INSERT를 수행했지만
-- VIEW 생성시 사용한 원본테이블 DEPT_COPY2에
-- 값이 INSERT 됨을 확인
...
-->CURRVAL의 정확한 의미는 가장 최근 호출된 NEXTVAL 값을
-->NEXTVAR를 호출한적이 없어서 오류 발생!!!

--> 해결방법 : NEXTVAL 호출하면 해결
SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL;
-- 시컨스 생성후 첫 NEXTVAL == STARR WITH 값이 100

SELECT SEQ_TEST_NO.CURRVALL FROM DUAL;
...
INSERT INTO TB_TEST
VALUES(SEQ_TEST_NO.NEXTVAL, '철수'); --125 짱구

INSERT INTO TB_TEST
VALUES(SEQ_TEST_NO.NEXTVAL, '유리'); --125 짱구
...
CREATE TABLE TB_IDX_TEST(
	TEST_NO NUMBER PRIMARY KEY, -- 자동으로 UNUQUE INDEX 생성
