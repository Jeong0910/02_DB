 ...
 * 
 * --테이블에서 수정할 수 있는 것
 * 1) 제약 조건(추가/삭제) --이미 있는 제약조건에서 수정 및 추가는 안된다
 * 					  -- 삭제 후에 추가해야함.
 * 2) 컬럼(추가/수정/삭제)
...
-- 컬럼 수정
-- ALTER TABLE 테이블명 MODIFY 컬러명 데이터타입; -> 데이터 타입 변경
...
--> 컬럼이 생성되면서 DEFAULT 값이 자동 삽입 되었음을 확인



-- D10 개발1팀 추가
INSERT INTO DEPT_COPY VALUES ('D10', '개발1팀', 'L1', DEFAULT, DEFAULT)
--  ORA-12899: "KH_JSW"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
--> DEPT_ID의 데이터 타입이 CAHR(2) 이므로 영어+숫자 2글자까지 저장 가능
--> D10 은 영어+숫자 3글자!!!
--> VARCHAR2(3)으로 변경해보기 - 메모리절약 CHAR는 메모리 남아있고, VARCHAR2는 반환
...
ALTER TABLE DEPT_COPY DROP COLUMN DEPT_ID; -- 마지막 남은 행은 삭제X
...
-- 3) DROP TABLE 삭제옵션 CASCADE CONSTRATINTS 사용
...
SELECT * FROM TB2; -- TB2테이블은 독립적인 테이블로 혼자 남게됨(아무관계없음)
...
--> DDL은 수행시 존재하고 있는 트랜잭션을 모두 DB 강제 COMMIT 시킴!
...
SELECT * FROM TB2;
-- 롤백 안된다. 위에서 DDL 구문 중 ALTER를 사용을 함으로써, 그 시점에 COMMIT됨
