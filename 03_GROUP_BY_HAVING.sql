...
-- 직급코드, 급여 평균, 인원 수를 (SELECT) COUNT행의 갯수 인원수
...
-- 별칭 사용 X (SELECT 해석이 되지 않아서)
ORDER BY "인원 수";
-- 별칭 사용 O (SELECT 절 해석이 완료 되어서)
...
GROUP BY DEPT_CODE;
-- 부서의(WHERE절에서 구한 부서코드가 D5,D6인 데이터들을 구릅으로 묶음)
...
--* 여러 컬럼을 묶어서 그룹을 지정 가능 --> 그룹 내 그룹이 가능하다! *
...
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE -- DEPT_CODE로 그룹화하고 나눠진 JOB_CODE로 그룹화
...
-- HAVING 절 : 그룹함수로 구해 올 그룹에 대핸 조건을 설정할 때 사용

-- EMPLOYEE 테이블에서
-- 부서별 평균 급여가 3백안원 이상인 부서의
-- 부서코드, 평균 급여 조회
-- 부서코드 오름차순 정렬
...
ORDER BY 1; -- SELECT에서 순서가 1 ->JOB_CODE
...
-- ORA-01790 :대응하는 식과 같은 데이터 유형이어야 한다
