...
CREATE USER jsw_sample IDENTIFIED BY sample1234;

-- 2. 새 연결(접속) 추가
-- 로그인 할 수 있는 권한이 없다는 오류메시지
--> 접속권한

-- 3. 접속 권한 부여 (sys -> 사용자계정)
-- [권한 부여 작성법]
-- GRANT 권한, 권한, ... TO 사용자명;
GRANT CREATE SESSION TO jsw_sample;
...
-- ORA-01031: 권한이 불충분합니다 - 현재 로그인할 권한밖에 없음
-- CREATE TABLE : 테이블 생성 권한
-- + 데이터를 저장할 수 있는 공간(TABLESPACE) 할당

-- 6. (SYS) 테이블 생성 권한 부여 + TABLESPACE 할당
GRANT CREATE TABLE TO jsw_sample;

ALTER USER jsw_sample DEFAULT TABLESPACE
SYSTEM QUOTA UNLIMITED ON SYSTEM;
-- SYSTEM 테이블스페이스를 사용자 jsw_sample의 
-- 저장소로 무제한 사용하게끔 하는 명령
...
GRANT CONNECT, RESOURCE TO jsw_sample;
...
GRANT SELECT ON EMPLOYEE TO jsw_sample;
...
-- 4. (KH) SAMPLE계정에 부여했던 EMPLOYEE 테이블 조회 권한 회수(REVOKE)
REVOKE SELECT ON EMPLOYEE FROM jsw_sample;
SELECT * FROM kh_jsw.EMPLOYEE;
-- 조회 불가능 (권한 회수 완료)
