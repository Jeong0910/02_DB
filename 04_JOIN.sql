...
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- 공통적인 연결고리가 있을시 가능
...
-- E J 말고 명칭 EMPLOYEE JOB 을 그대로 넣어서 작성해도 됨.
...
FROM EMPLOYEE LEFT OUTER JOIN DEPARTMENT-- 위치가 다르면 결과값이 다르다.
...
-- RIGHT [OUTER] JOIN : 합치게 사용한 두테이블 중
...
-- 3. 교차 조인(CROSS JOIN == CARTSTIAN PRODUCT)
...
-- LEFT를 빼버리면 조회자체가 안된다. -> OUTER JOIN으로 됨
...
-- 임플로이랑 디파트먼트 조인하고 그다음 로케이션 조인한다.
-- 3개다 겹치는 것은 없다.
...
-- ANSI 표준
SELECT EMP_ID, EMP_NAME, JOB_CODE, LOCAL_NAME, SALARY
FROM EMPLOYEE 
JOIN JOB USING (JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
...
SELECT EMP_NAME 이름, JOB_NAME 직급, SALARY 급여, SALARY*(1+NVL(BONUS,0))*12 연봉
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
...
SELECT E1.EMP_NAME 사원, E1.DEPT_CODE 부서코드, E2.EMP_NAME 동료
FROM EMPLOYEE E1
FROM EMPLOYEE E2 ON(E1.DEPT_CODE = E2.DEPT_CODE)
