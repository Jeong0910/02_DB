...
SELECT DEPT_CODE
FROM EMPLOYEE
WHERE EMP_NAME = '노옹철'; -- D9 메인쿼리의 조건으로 사용한다
...
--> 위의 2개 단계를 하나의 쿼리로!
SELECT EMP_NAME, DEPT_CODE -- 외부쿼리
FROM EMPLOYEE
WHERE DEPT_CODE = (SELECT DEPT_CODE -- 내부쿼리
					FROM EMPLOYEE
					WHERE EMP_NAME = '노옹철');
...
-- 차장 직급의 급여중 가장 큰값보다 많이 받는 과장 직급의 직원
-- 사번, 이름, 직급, 급여 조회
...
-- 3) EMPLOYEE 테이블의 위의 결과와 동일한 DEPT_ID를 가진
-- 사원의 이름, 부서코드 조회
...
-- 사번, 이름, 직급코드, 급여 조회
-- 단, 급여와 급여 평균은 만원 단위로 계산 TRUNC(컬럼명, 단위) 만원단위 -4
...
SELECT EMP_NAME, JOB_CODE, SALARY,
( SELECT MAX(SALARY) FROM EMPLOYEE) - SALARY "급여차"
FROM EMPLOYEE;
...
FROM (SELECT EMP_NAME, SALARY
	  FROM EMPLOYEE
	  ORDER BY SALARY DESC) -- 해석순위1위 FROM 절에서 전체직원의 SALARY 내림차순 정렬
...
-- RANK() OVER : 동일한 순위 이후의 등수를 동일한 이원 수 만큼 건너 뛰고 순위 계산
